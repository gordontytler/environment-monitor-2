...
public class InputFromSSHReader implements Runnable {
	...
	private PasswordTyper passwordTyper = new PasswordTyper();
	private SSHExecuter sshExecuter;
	...
	void addCommandToQueue(int commandNumber, Command command) {
		if (logger.isLoggable(Level.FINE)) logger.fine(String.format("sessionId:%s put command %d - '%s' on blocking queue .", sessionId, commandNumber, command));
	...
	CommandAndNumber takeNextCommandAndNumber() throws InterruptedException {
		if (logger.isLoggable(Level.FINE)) logger.fine(String.format("sessionId:%s getting next command from blocking queue...", sessionId));
		CommandAndNumber commandAndNumber = commandQueue.take(); // <-- it blocks here
		if (stopSignal != null || stopFinishedCheckingSignal != null) {
			if (logger.isLoggable(Level.FINE)) logger.fine(String.format("sessionId:%s stoppedSignal is %b stopFinishedCheckingSignal is %b", sessionId, (stopSignal != null), (stopFinishedCheckingSignal != null)));
			return null;
		}
		...
		if (logger.isLoggable(Level.FINE)) logger.fine(String.format("sessionId:%s[%d] - '%s' taken from blocking queue.", sessionId, commandNumber, runningRequest));
	...
	public void run() {
		try {
			...
			while (stopSignal == null) {
				if (logger.isLoggable(Level.FINE)) logger.fine(String.format("sessionId:%s[%d] Looking for prompt: %s", sessionId,  commandNumber, sshExecuter.getPrompt()));
				String output = "";
				...
				if (logger.isLoggable(Level.FINE)) logger.fine(String.format("sessionId:%s[%d] foundPrompt=%b stopLookingForPrompt=%b - %s", sessionId, commandNumber, foundPrompt, stopFinishedCheckingSignal == null ? false : true, safeSubString(runningRequest, 30)));
			...
			if (logFine) logger.info(String.format("sessionId:%s[%d] InputFromSSHReader thread ending normally.", sessionId, commandNumber));
	...
	String filter(int len, byte[] buff, Command command) throws Exception {
		...
		if (logFineFilter) logger.info(String.format("sessionId:%s[%d] raw buffer: %s", sessionId, commandNumber,  new String(buff, 0 ,len)));
		...
		if (len > promptLength && buff[0] == promptBytes[0] && buff[promptLength -1] == promptBytes[promptLength -1]) {
			...
			if (startsWithPrompt) {
				...
				if (logFineFilter) logger.info(String.format("sessionId:%s[%d] Removed a prompt at beginning that is followed by other stuff.", sessionId, commandNumber));
			}
		}
		...
		if (logFineFilter) logger.info(String.format("sessionId:%s[%d] foundPrompt %b offset %d lastNewline %d len %d : %s ", sessionId, commandNumber, foundPrompt, offset, lastNewline, len, filtered));
	...
	public void changeSessionId(String newSessionId) {
		if (logFine) logger.info(String.format("sessionId:%s this sessionId has been replaced with new sessionId:%s", sessionId, newSessionId));
