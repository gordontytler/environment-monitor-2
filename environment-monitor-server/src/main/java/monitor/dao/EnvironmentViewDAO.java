...
public class EnvironmentViewDAO {
    ...
    public static final String ENVIRONMENTS = "environments" + File.separatorChar;
    private static final EnvironmentViewDAO theInstance = new EnvironmentViewDAO();
    static Logger logger = Logger.getLogger(EnvironmentViewDAO.class.getName());
    private final ApplicationCache applicationCache = ApplicationCache.getInstance();
    ...
    private EnvironmentView load(String environmentName) {
        ...
        try {
            String fileName = getPath() + ENVIRONMENTS + FileDAOHelper.toFileName(environmentName) + ".txt";
    ...
    public synchronized void renameEnvironmentView(String oldName, String newName) {
        ...
        if (oldView != null) {
            ...
            oldView.setFileName(ENVIRONMENTS + FileDAOHelper.toFileName(newName) + ".txt");
    ...
    public synchronized void saveEnvironmentView(EnvironmentView environmentView) {
        String fileName = ENVIRONMENTS + FileDAOHelper.toFileName(environmentView.getEnvironmentName()) + ".txt";
        try {
            ...
            File file = new File(getPath() + ENVIRONMENTS + FileDAOHelper.toFileName(environmentView.getEnvironmentName()) + ".txt");
            ...
            throw new MonitorRuntimeException("Problem saving environment view: \"" + environmentView.getEnvironmentName() + "\". File name: " + fileName, e);
        }
    ...
    public synchronized void deleteEnvironmentView(String environmentName) {
        try {
            File file = new File(ENVIRONMENTS + FileDAOHelper.toFileName(environmentName) + ".txt");
            file.delete();
    ...
    private String getDefaults() {
        return ENVIRONMENTS + "defaults.txt";
    }
