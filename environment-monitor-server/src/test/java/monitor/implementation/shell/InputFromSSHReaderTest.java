...
import monitor.implementation.session.*;
...
public class InputFromSSHReaderTest {
    ...
    public static void setUp() throws Exception {
        ...
        // The prompt for any user will depend on how the user is set up
        // todo need a setup script for tests to create users
        String prompt = executer.getPrompt();
        if ("auto".equals(sshConnection.getLoggedOnUserName())) {
            String userAtHost = "auto@" + host + ":/$ ";
            if (!prompt.equals("$ ") && !prompt.equals(userAtHost)) {
    ...
    */
    ...
    /*
        I should ignore this test and remove the code that creates users.
        Jun 11, 2020 12:24:00 PM monitor.implementation.shell.InputFromSSHReader readInputFromShell
        INFO: sessionId:2[0] Password provided for user gordon running command: sudo /usr/sbin/useradd -G gordon,sudo -K PASS_MAX_DAYS=-1 au...
    */
    @Ignore
    @Test
